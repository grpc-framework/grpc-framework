...
func RegisterHelloGreeterHandlerServer(ctx context.Context, mux *runtime.ServeMux, server HelloGreeterServer) error {
	mux.Handle(http.MethodPost, pattern_HelloGreeter_SayHello_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
...
func RegisterHelloIdentityHandlerServer(ctx context.Context, mux *runtime.ServeMux, server HelloIdentityServer) error {
	mux.Handle(http.MethodGet, pattern_HelloIdentity_ServerVersion_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		ctx, cancel := context.WithCancel(req.Context())
		defer cancel()
		var stream runtime.ServerTransportStream
		ctx = grpc.NewContextWithServerTransportStream(ctx, &stream)
...
// "HelloGreeterClient" to call the correct interceptors. This client ignores the HTTP middlewares.
...
// doesn't go through the normal gRPC flow (creating a gRPC client etc.) then it will be up to the passed in
// "HelloIdentityClient" to call the correct interceptors. This client ignores the HTTP middlewares.
